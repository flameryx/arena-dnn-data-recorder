...
class RecordedAverage:
    ...
    def createAverages(args):
        ...
        numIterations = args.map_iterations
        ...
        RecordedAverage.combineAveragesAndWorldComplexity(averagesCSVOutput, outputPath, numIterations)
    ...
    def combineAveragesAndWorldComplexity(averagesData, outputPath, numIterations):
        ...
        if numIterations != None:
            combinedDataFrame["numIterations"] = numIterations
    ...
    def calculateEpisodeAverage(data):
        ...
        if(exists_collision):
            data['done_reason'] = 0
            data["collision"] = 1
            # collision can only happen after before a timeout. 
            # If a collsion and a timeout accured then the collsion was
            # always before the timeout so the timeout will be discarded
            data["timeout"] = 0
        ...
        outputPath = "episode_averages"
        path = pl.Path(outputPath)
        path.mkdir(parents=True, exist_ok=True)

        csvFilename = "episode_averages/{}.csv".format(currentMapName)
        with open(csvFilename, 'a') as f:
            averageDataFrameExtended.to_csv(f, mode='a', header=f.tell()==0, index=False)
...
if __name__ == "__main__":
    ...
    dest="map_iterations",
    default=None,
    help="specifies with how many iterations was set for the map creator",
    required = False
    )
